...
<template>
  <div>
    <table border="1" cellspacing="0" cellpadding="0" style="border-collapse: collapse; width: 100%;">
      <thead>
        <tr>
          <th style="text-align:left; width:100px; padding:4px;">Floor\Room</th>
          <th
            v-for="c in COLS"
            :key="c"
            style="text-align:center; min-width:80px; height:40px; padding:4px;"
          >
            {{ c }}
          </th>
        </tr>
      ...
      <tbody>
        ...
        <tr v-if="Object.keys(processed.wholeRow).length">
          <td><strong>Whole Unit</strong></td>
          <td
            v-for="c in COLS"
            :key="'whole-'+c"
        ...
        <!-- 没有房 -->
        <tr v-if="!processed.floors.length && !Object.keys(processed.otherRow).length && !Object.keys(processed.wholeRow).length">
