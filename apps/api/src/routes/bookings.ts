...
/** 列表：支持 一堆过滤 + 分页 */
r.get("/", async (req, res) => {
  const { propertyId, roomId, guestId, channel, status, from, to, q } = req.query as any;
  ...
  if (q) {
    where.OR = [
      ...
      { guest: { name: { contains: String(q), mode: "insensitive" } } as any },
    ];
  }
...
r.post("/", async (req, res) => {
  ...
  if (!roomId || !guestId || !checkIn || !checkOut) return res.status(400).json({ error: "roomId/guestId/checkIn/checkOut required" });
...
r.delete("/", async (req, res) => {
  const { propertyId, before } = req.query as any;
  const where: any = {};
  if (propertyId) where.room = { propertyId: String(propertyId) };
  if (before) where.checkIn = { lt: new Date(String(before)) };
