...
function parseTable(filePath: string): Row[] {
  ...
  for (const [label, v] of Object.entries({ pIdx, rIdx, nIdx, miIdx, moIdx })) {
    if (v === -1) throw new Error('缺少必需列：Property, Room, Name, Move in, Move out');
  }
...
async function main() {
  ...
  // 需要的 propertyName 集合
  const wantedPropertyNames = new Set<string>();
  const wantedRoomPairs: Array<{ propertyName: string; label: string }> = [];
  ...
  const roomKeyToId = new Map<string, string>(); // key = `${propertyId}::${label}`
  for (const r of rooms) roomKeyToId.set(`${r.propertyId}::${r.label}`, r.id);
  ...
  await Promise.allSettled(
    rows.map(r => limit(async () => {
      try {
        ...
        if (!propertyId) { skip++; notFound.push(`Property "${propertyName}"`); bar.increment({ ok, skip, fail }); return; }

        const label = r.room.trim();
        const roomId = roomKeyToId.get(`${propertyId}::${label}`);
        if (!roomId) { skip++; notFound.push(`Room "${propertyName}" / "${label}"`); bar.increment({ ok, skip, fail }); return; }

        const guestId = nameToGuestId.get(r.name.trim());
        if (!guestId) { skip++; notFound.push(`Guest "${r.name}"`); bar.increment({ ok, skip, fail }); return; }
  ...
  if (newBookingIds.length) {
    const bar2 = new cliProgress.SingleBar(
      { format: '过账进度 |{bar}| {percentage}% | {value}/{total}' },
      cliProgress.Presets.shades_classic
    );
    ...
    await Promise.allSettled(
      newBookingIds.map(id => postLimit(async () => {
