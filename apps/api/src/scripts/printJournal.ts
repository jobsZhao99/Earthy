...
type Args = {
  from?: string; // YYYY-MM
  to?: string;   // YYYY-MM
  ledger?: string;    // ledger 名称（优先）
  ledgerId?: string;  // 或指定 id
  out?: string;       // 输出文件名，默认 journal-monthly.csv
  fmt?: 'csv' | 'xlsx';
};
...
async function resolveLedger(ledger?: string, ledgerId?: string) {
  if (ledgerId) return { id: ledgerId, name: null as string | null };
  if (!ledger) return null;
  const row = await prisma.ledger.findFirst({ where: { name: ledger }, select: { id: true, name: true } });
  if (!row) throw new Error(`找不到 Ledger 名称：${ledger}`);
  return { id: row.id, name: row.name };
}

function monthStartUTC(ymStr: string): Date {
...
async function main() {
  ...
  if (ledgerFilter) whereEntry.ledgerId = ledgerFilter.id;
  if (args.from) {
  ...
  if (args.fmt === 'xlsx') {
    if (!XLSX) throw new Error('未安装 xlsx，请运行: npm i -D xlsx');
