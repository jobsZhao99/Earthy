...
function parseTable(filePath) {
    ...
    for (const [k, v] of Object.entries(required)) {
        if (v === -1)
            throw new Error(`缺少必需列：${k.replace('Idx', '').replace(/[A-Z]/g, m => ' ' + m).trim()}（需要 Property, Room, Name, Move in, Move out）`);
    }
    const rows = [];
    for (let i = 1; i < lines.length; i++) {
        ...
        if (!property || !room || !name || !moveIn || !moveOut)
            continue;
        rows.push({
            property,
            unit,
...
async function ensureGuestByName(name) {
...
async function main() {
    ...
    for (const row of rows) {
        try {
            ...
            const memoParts = [];
            if (row.note)
                memoParts.push(row.note);
            if (row.confirmedDate)
                memoParts.push(`Confirmed: ${row.confirmedDate}`);
